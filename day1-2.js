...
const first = input
  .split("\n")
  .map((arr) =>
    arr.match(/(?:\d|one|two|three|four|five|six|seven|eight|nine)/)
  )
  .map((arr) => arr[0])
  .map((i) => total += (i.length == 1 ? Number(i) : numsword.indexOf(i)+1)*10);

const last = input
  .split("\n")
  .map((arr) => arr.split("").reverse().join(""))
  .map((arr) =>
    arr.match(/(?:\d|enin|thgie|neves|xis|evif|ruof|eerht|owt|eno)/)
  )
  .map((arr) => arr[0].split("").reverse().join(""))
  .map((i) => total += (i.length == 1 ? Number(i) : numsword.indexOf(i)+1));
