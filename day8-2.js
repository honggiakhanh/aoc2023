...
let path = [];
input.map((row) => {
  path.push(row.match(/\w+/g));
});

let pathMap = new Map();

path.map((row) => {
  pathMap.set(row[0], [row[1], row[2]]);
});
...
for (const [key, value] of pathMap) {
  if (key.endsWith("A")) {
    ...
    while (!currentKey.endsWith("Z")) {
      currentKey =
        pathMap.get(currentKey)[instruction[stepCounter % instruction.length]];
      stepCounter++;
    }
    loopLengths.push(stepCounter);
  }
}
