...
numberArr.map((nrow, r) => {
  nrow.map((num) => {
    for (i = -1; i < 2; i++) {
      if ((r == 0 && i == -1) || (r == input[r].length - 1 && i == 1)) {
        continue;
      }
      let block = input[r + i].slice(
        Math.max(num.index - 1, 0),
        Math.min(num.index + num[0].length + 1, input[r + i].length - 1)
      );
      if (block.match(/[^\d\.]/)) {
        total += Number(num);
      }
